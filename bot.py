"""
魅物AI Telegram Bot — Render 部署版
- 使用 Webhook 模式（替换 run_polling，适配 Render）
- 监听 $PORT 环境变量（Render 要求）

Render 环境变量配置（在 Render Dashboard → Environment 中设置）：
  BOT_TOKEN        = 你的Bot Token
  WEBHOOK_URL      = https://你的应用名.onrender.com   （不加末尾斜杠）
  COMFYUI_BASE     = https://你的ComfyUI地址
  PAYMENT_API_URL  = https://你的支付API地址
...
#  导入
# ============================================================
import os, json, uuid, time, asyncio, logging, io, base64
import requests
...
WEBHOOK_PATH    = f"/webhook/{BOT_TOKEN}"             # 安全路径，避免被猜到
...
async def comfyui_generate(workflow: dict, timeout: int = 120) -> Optional[bytes]:
    ...
    try:
        ...
        if r.status_code != 200:
            logger.error(f"提交任务失败: {r.status_code} {r.text[:200]}")
            return None
...
async def finish_image(update: Update, context: ContextTypes.DEFAULT_TYPE,
                       img_bytes: Optional[bytes], prompt: str):
    ...
    if img_bytes:
        ...
        await msg.reply_text(
            "❌ 生图失败，请确认：\n"
            "1. CloudStudio 会话仍在运行\n"
            "2. ComfyUI 已正常启动\n"
            "3. models/checkpoints/ 下有模型文件"
        )
    await msg.reply_text(DISCLAIMER)
    await msg.reply_text(f"📊 剩余次数：{u['images_left']} 次",
                         reply_markup=main_keyboard())

async def complete_payment(uid: int, pkg_key: str,
...
# ============================================================
#  文生图 ConversationHandler
# ============================================================
async def txt2img_entry(update: Update, context: ContextTypes.DEFAULT_TYPE):
...
async def txt2img_do(update: Update, context: ContextTypes.DEFAULT_TYPE):
    uid = update.effective_user.id
    u = get_user(uid)
    prompt = update.message.text.strip()
    if u["images_left"] < 1:
        await update.message.reply_text("⚠️ 次数不足，请购买套餐", reply_markup=main_keyboard())
        return ConversationHandler.END
...
# ============================================================
#  图生图 ConversationHandler
# ============================================================
async def img2img_entry(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    u = get_user(query.from_user.id)
    if u["images_left"] < 1:
...
async def img2img_got_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
...
async def img2img_do(update: Update, context: ContextTypes.DEFAULT_TYPE):
    uid = update.effective_user.id
    u = get_user(uid)
    prompt = update.message.text.strip()
...
# ============================================================
#  机器人克隆 ConversationHandler
# ============================================================
async def clone_entry(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    await query.message.reply_text(
        "🤖 *克隆机器人*\n\n"
...
async def clone_got_token(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        asyncio.get_event_loop().create_task(launch_cloned_bot(token, uid))
...
async def launch_cloned_bot(token: str, owner_id: int):
    try:
...
async def cb_buy_package(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    kb = [
...
async def cb_user_center(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    uid = query.from_user.id
    u = get_user(uid)
...
async def cb_ai_partner(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    await query.message.reply_text(
        "🚀 【AI合伙人代理 · 收益规则】\n\n"
...
# ============================================================
#  ★ 主入口 — Webhook 模式（Render 专用）
# ============================================================
def main():
    app = Application.builder().token(BOT_TOKEN).build()
    setup_all_handlers(app)

    async def on_startup(a: Application):
        ...
        logger.info(f"🎨 ComfyUI    = {COMFYUI_BASE}")

    app.post_init = on_startup

    # ★ 核心修复：run_webhook 替换 run_polling
    app.run_webhook(
        listen="0.0.0.0",        # 监听所有网卡
        port=PORT,                # 使用 Render 注入的 $PORT
        url_path=WEBHOOK_PATH,    # 安全的 webhook 路径
        webhook_url=WEBHOOK_FULL, # 向 Telegram 注册的完整 URL
